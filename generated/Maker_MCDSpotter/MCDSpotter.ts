...
export class Poke extends EthereumEvent {
  get params(): Poke__Params {
    return new Poke__Params(this);
  }
}

export class Poke__Params {
  _event: Poke;

  constructor(event: Poke) {
    this._event = event;
  }
...
export class MCDSpotter__ilksResult {
  value0: Address;
  value1: BigInt;
...
export class MCDSpotter extends SmartContract {
  static bind(address: Address): MCDSpotter {
    return new MCDSpotter("MCDSpotter", address);
  }

  ilks(param0: Bytes): MCDSpotter__ilksResult {
    let result = super.call("ilks", [EthereumValue.fromFixedBytes(param0)]);

    return new MCDSpotter__ilksResult(
      result[0].toAddress(),
      result[1].toBigInt()
    );
  }

  try_ilks(param0: Bytes): CallResult<MCDSpotter__ilksResult> {
    ...
    return CallResult.fromValue(
      new MCDSpotter__ilksResult(value[0].toAddress(), value[1].toBigInt())
    );
  }
  ...
  par(): BigInt {
    let result = super.call("par", []);

    return result[0].toBigInt();
  }

  try_par(): CallResult<BigInt> {
    let result = super.tryCall("par", []);
    if (result.reverted) {
      return new CallResult();
    }
    let value = result.value;
...
export class PokeCall extends EthereumCall {
